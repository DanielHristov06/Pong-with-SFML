...
void CpuPaddle::Update(sf::Time dt, Ball& ball) {
	...
	if (mShape.getPosition().y + (float)mHeight / 2 > ball.getPosition().top + ball.getPosition().height){
		mPosition.y -= mSpeed * dt.asSeconds();
	}
	else if (mShape.getPosition().y + (float)mHeight / 2 <= ball.getPosition().top + ball.getPosition().height) {
		mPosition.y += mSpeed * dt.asSeconds();
	}
